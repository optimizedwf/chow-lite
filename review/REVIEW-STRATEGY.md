# chow-lite — All Things Agentic 2026: Score-Maximization Strategy
...
| Requirement (rules §6) | chow-lite | Verdict |
|---|---|---|
| Gemini 3.5 or newer via **Gemini API or Vertex AI** | `gemini-3.5-flash` via `google-genai` client (router: `chowlite/router/classifier.py`; live tests: `tests/test_router_live.py`; ADK agent: `tests/test_adk.py`) | ✅ verified in code |
| At least one **Google agent framework** (ADK, GenAI SDK, Antigravity, GenKit) | `google-adk==2.6.3` installed; `chowlite/runtime/adk_runtime.py` (LlmAgent, FunctionTool, InMemoryRunner); live test `tests/test_adk.py` | ⚠️ **ADK exists but is NOT invoked in any user-facing path** — see §2 row 4. This is the weakest spot. |
| At least one **Google Cloud infra service** | Cloud Run (`deploy/cloud-run.yaml`, `Dockerfile`, `deploy/deploy.sh`) + Firestore (`deploy/firestore.rules`, `chowlite/ledger/firestore_ledger.py`) | ✅ in code; **deployment itself still pending (Adam step)** — see §4-R1 |
...
| URL to hosted project (if available) — *highly encouraged* | ⏳ after deploy (`chow-lite-<hash>.run.app`) |
| Text description (features, tech, findings) | ✅ SUBMISSION.md §3 |
| URL to repo (public or private + access) | ✅ public MIT `github.com/optimizedwf/chow-lite` |
...
| 2 | "~1,200 lines of Python" (SUBMISSION.md) | Product code = **2,293 LOC** (chowlite/ + deploy/ + workflows/ + demo) excl. tests | Update or scope ("core loop ~1,200 LOC, ~2,300 total") |
...
| R4 | **Team eligibility**: every member must be an eligible individual on Devpost | 🟠 | List only humans (solo `optimizedworkflowdev` is cleanest). Do **not** list an AI agent as a teammate. LICENSE copyright line "Adam Norman & Chow" is fine. |
...
| R6 | **New-project rule / pre-existing code**: internal chow harness could look pre-existing | 🟠 | All commits in-window ✅; keep the "rebuilt fresh... concepts, not code" disclosure; add an explicit disclosure line in SUBMISSION.md |
...
| R8 | **Deployed API not actually using Gemini** (no key in env) → video claim false | 🟠 | Fix `deploy.sh`; verify `/health` and a submit show `router: gemini-3.5-flash-live` |
...
- Replace the canned `build` bash node in `chowlite/chains/flagship.py` with an **ADK `LlmAgent`** (`model=Gemini(model="gemini-3.5-flash")`, `tools=[FunctionTool(run_python), FunctionTool(read_task)]`) that reads `task.txt`+`PLAN.md`, writes real `solution.py`, and runs it.
...
- Add `chow learn apply <candidate_id>`: runs the regression suite (`pytest tests/ -q -k "not live"`), then writes the router keyword/description change into a git-tracked catalog file (`chowlite/router/catalog.json`) and commits; `chow learn revert <candidate_id>` restores. Rollback = git revert.
...
- 30-second takeaway for judges: **"Most agents say 'done' with no proof. chow-lite is an agent OS that refuses to ship without evidence — it routes, executes, verifies, and learns, live on Google Cloud — and it gets smarter every run."**
...
1. **Title** — current 67 chars > Devpost 60-char limit. Use:
   **"chow-lite — the agent OS that proves its work"** (43) or **"chow-lite — an evidence-gated agent OS"** (38).
...
| 3 | **P2: LEARN end-to-end** (persistent events, keyword candidates, `chow learn apply` w/ regression+rollback, `/v1/events`) | HIGH | M (1.5–2 d) | Week 2 |
...
### Do this week: top 5
1. **Deploy** — `gcloud auth login` → fix `deploy/deploy.sh` (add `GEMINI_API_KEY` from secret) → `bash deploy/deploy.sh` → confirm `/health` and a live submit show `router: gemini-3.5-flash-live`. (Adam, ~30 min; everything else waits on this)
