...
managed with the same route→execute→verify→learn loop. 616/621 tests pass (621 collected, 5 live-gated skips);
...
| Demo & Production Readiness (30%) | one-command demo, 616 tests (621 collected), Dockerfile + deploy.sh, FastAPI operator API, model-or-fail (no fabricated output) |
