...
managed with the same route→execute→verify→learn loop. 481/486 tests pass (486 collected, 5 live-gated skips);
...
| Demo & Production Readiness (30%) | one-command demo, 481 tests (486 collected), Dockerfile + deploy.sh, FastAPI operator API, model-or-fail (no fabricated output) |
