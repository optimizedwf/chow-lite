...
managed with the same route→execute→verify→learn loop. 617/622 tests pass (622 collected, 5 live-gated skips);
...
| Demo & Production Readiness (30%) | one-command demo, 617 tests (622 collected), Dockerfile + deploy.sh, FastAPI operator API, model-or-fail (no fabricated output) |
