...
managed with the same route→execute→verify→learn loop. 459/464 tests pass (464 collected, 5 live-gated skips);
...
| Demo & Production Readiness (30%) | one-command demo, 459 tests (464 collected), Dockerfile + deploy.sh, FastAPI operator API, model-or-fail (no fabricated output) |
