...
managed with the same route→execute→verify→learn loop. 596/601 tests pass (601 collected, 5 live-gated skips);
...
| Demo & Production Readiness (30%) | one-command demo, 596 tests (601 collected), Dockerfile + deploy.sh, FastAPI operator API, model-or-fail (no fabricated output) |
