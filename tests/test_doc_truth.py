...
def test_readme_test_counts_are_current():
    ...
    t = _readme()
    assert "tests-583%20passing" in t, "badge stale (expected 583 passing)"
    assert "583 passing tests (588 collected" in t, "roadmap claim stale"
    assert "588 tests (router, ledger" in t, "tree count stale"

def test_submission_test_counts_are_current():
    s = _submission()
    assert "583/588 tests pass" in s, "submission loop claim stale"
    assert "583 tests (588 collected)" in s, "submission readiness claim stale"
