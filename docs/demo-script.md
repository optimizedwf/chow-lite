# Demo video script — nine (final: ~2:25, ≤4:00 cap, public YouTube, LIVE GCP PROOF required)
...
| 0:03–1:10 | Terminal demo: live ROUTE (Gemini 3.6 Flash, conf 1.00) → 5-hop EXECUTE → VERIFY artifacts → LEARN events; then Taskmaster lane | seg_terminal.mp4 (66.8s, VO mixed) | "First, route..." (GuyNeural, already mixed) |
...
| 1:57–2:22 | **LIVE GCP PROOF** — `nine probe https://nine-<id>.run.app`: /health, /v1/submit x2 (real Gemini routing on the deployed API, both SHIP), /v1/jobs, /v1/stats (Firestore-backed) | gcp_segment.mp4 (record after deploy) | "And here is the same system, live on Google Cloud..." |
...
2. `python deploy/demo_probe.py https://nine-XXXX.run.app | tee demo_capture/gcp_transcript.txt`
...
5. Upload PUBLIC to YouTube (title: "nine — an evidence-gated agent OS (Google ADK + Gemini 3.6 Flash)"), paste URL into Devpost submission.
